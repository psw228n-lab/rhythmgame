...
var DIFFICULTY_LIMITS = {
	...
	normal: {
		minGap: .14,
		maxNotesPerSecond: 4.5
	},
	hard: {
		minGap: .075,
		maxNotesPerSecond: 7.2
	}
};
var LANE_CODES = [
	"KeyZ",
...
//#endregion
//#region src/game/chartUtils.ts
function validateChart(value) {
	if (!value || typeof value !== "object") return false;
	const chart = value;
	if (typeof chart.title !== "string" || typeof chart.audio !== "string" || typeof chart.offset !== "number" || typeof chart.bpm !== "number" || ![
		"easy",
		"normal",
		"hard"
	].includes(chart.difficulty ?? "") || !Array.isArray(chart.notes)) return false;
	return chart.notes.every((note) => note && typeof note.time === "number" && Number.isFinite(note.time) && Number.isInteger(note.lane) && note.lane >= 0 && note.lane <= 3 && (note.type === "tap" || note.type === "hold") && (note.type === "tap" || typeof note.duration === "number" && note.duration > 0));
}
function postProcessNotes(notes, difficulty, duration = Number.POSITIVE_INFINITY) {
	const { minGap, maxNotesPerSecond } = DIFFICULTY_LIMITS[difficulty];
	const sorted = notes.filter((note) => Number.isFinite(note.time) && note.time >= 1.5 && note.time < duration - .15 && Number.isInteger(note.lane) && note.lane >= 0 && note.lane <= 3 && (note.type === "tap" || note.type === "hold")).sort((a, b) => a.time - b.time || a.lane - b.lane);
	const result = [];
	const recentTimes = [];
	let sameLaneRun = 0;
	let previousLane = -1;
	for (const source of sorted) {
		const note = {
			...source,
			time: Number(source.time.toFixed(4))
		};
		if (result.some((item) => item.lane === note.lane && Math.abs(item.time - note.time) < .035)) continue;
		if (result.filter((item) => Math.abs(item.time - note.time) < .018).length >= 2) continue;
		const lastDifferentTime = [...result].reverse().find((item) => Math.abs(item.time - note.time) >= .018)?.time;
		if (lastDifferentTime !== void 0 && note.time - lastDifferentTime < minGap) continue;
		...
		if (note.type === "hold") {
			const safeDuration = Math.max(.28, Math.min(note.duration ?? .5, duration - note.time - .1));
			note.duration = Number(safeDuration.toFixed(3));
		} else delete note.duration;
		result.push(note);
...
//#endregion
//#region src/editor/ChartEditor.tsx
var import_jsx_runtime = require_jsx_runtime();
function ChartEditor({ audio, chart, onChartChange, onMessage }) {
	const canvasRef = (0, import_react.useRef)(null);
	const fileRef = (0, import_react.useRef)(null);
	const [notes, setNotes] = (0, import_react.useState)(() => chart.notes.map((note) => ({ ...note })));
	...
	const [offset, setOffset] = (0, import_react.useState)(chart.offset);
	const [now, setNow] = (0, import_react.useState)(audio.currentTime);
	(0, import_react.useEffect)(() => {
		setNotes(chart.notes.map((note) => ({ ...note })));
		setOffset(chart.offset);
		setSelected(null);
	}, [chart]);
	const commit = (0, import_react.useCallback)((nextNotes, nextOffset = offset) => {
	...
	(0, import_react.useEffect)(() => {
		...
		const render = () => {
			...
			if (loop && audio.currentTime >= Math.floor(audio.currentTime / 4) * 4 + 3.95) audio.seek(Math.floor(audio.currentTime / 4) * 4);
	...
	(0, import_react.useEffect)(() => {
		const onKeyDown = (event) => {
			...
			const lane = LANE_CODES.indexOf(event.code);
			if (lane < 0) return;
			event.preventDefault();
			const note = {
				time: Number(audio.currentTime.toFixed(4)),
				lane,
				type: "tap"
			};
	...
	const handleCanvasClick = (event) => {
		...
		const closest = notes.map((note, index) => ({
			index,
			distance: Math.abs(note.time - targetTime),
			lane: note.lane
		})).filter((item) => item.lane === lane && item.distance < .12).sort((a, b) => a.distance - b.distance)[0];
		if (closest) {
			setSelected(closest.index);
			return;
		}
		const next = [...notes, {
			time: Number(targetTime.toFixed(4)),
			lane,
			type: "tap"
		}];
		commit(next);
	...
	const updateSelected = (patch) => {
		...
		commit(notes.map((note, index) => index === selected ? {
			...note,
			...patch
		} : note));
	};
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "eyebrow",
					children: "LIVE CHART LAB"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "채보 에디터" })] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "toolbar-actions",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							className: "button button-primary",
							onClick: togglePlayback,
							children: playing ? "일시정지" : "재생"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
							className: "compact-field",
							children: ["속도", /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
								value: playbackRate,
								onChange: (event) => {
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							className: `button ${loop ? "is-active" : ""}`,
							onClick: () => setLoop((value) => !value),
							children: ["4초 반복 ", loop ? "ON" : "OFF"]
						})
					]
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("canvas", {
					...
					"aria-label": "클릭해서 노트를 추가하는 타임라인"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "timeline-playhead" })]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "timeline-meta",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatTime$1(now) }),
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatTime$1(audio.duration) })
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "Z · X · C · V를 음악과 함께 눌러 기록하거나 타임라인을 클릭하세요." }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "lane-mini-grid",
								children: LANE_LABELS.map((label, lane) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									style: { "--lane-color": LANE_COLORS[lane] },
									children: label
								}, label))
							}),
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						...
						}), selectedNote ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("strong", { children: [
								selectedNote.time.toFixed(3),
								"초 · ",
								LANE_LABELS[selectedNote.lane]
							] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "nudge-row",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										onClick: () => updateSelected({ time: Math.max(0, selectedNote.time - .01) }),
										children: "−10ms"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										onClick: () => updateSelected({ time: selectedNote.time + .01 }),
										children: "+10ms"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										className: "danger",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "compact-field",
								children: ["레인", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("select", {
									value: selectedNote.lane,
									onChange: (event) => updateSelected({ lane: Number(event.target.value) }),
									children: LANE_LABELS.map((label, lane) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
										value: lane,
										children: label
									}, label))
								})]
							...
							className: "muted",
							children: "노트를 선택하면 미세 조정 도구가 표시됩니다."
						})]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "editor-card",
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "compact-field",
								children: ["전체 오프셋", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
									type: "number",
									step: .01,
									value: offset,
									onChange: (event) => {
...
function drawTimeline(canvas, notes, currentTime, selected) {
...
//#endregion
//#region src/game/AudioManager.ts
var AudioManager = class {
	...
	async load(url) {
		...
		await new Promise((resolve, reject) => {
			const ready = () => resolve();
			const failed = () => reject(/* @__PURE__ */ new Error("음악 파일을 불러오지 못했습니다. public/audio/song.mp3를 확인해 주세요."));
	...
	setVolume(value) {
		if (this.gain && this.context) this.gain.gain.setTargetAtTime(value, this.context.currentTime, .015);
...
//#endregion
//#region src/game/JudgeManager.ts
var createScoreState = () => ({
	score: 0,
	combo: 0,
	maxCombo: 0,
	counts: {
...
function calculateAccuracy(state) {
	...
	if (!total) return 100;
	return Object.keys(state.counts).reduce((sum, key) => sum + state.counts[key] * ACCURACY_WEIGHTS[key], 0) / total * 100;
}
...
var GameEngine = class {
	...
	press(lane, audioTime) {
		...
		const deltaMs = (audioTime - candidate.time) * 1e3;
	...
	update(audioTime, heldLanes) {
		...
		for (let index = this.nextMissIndex; index < this.notes.length; index += 1) {
			...
			if (note.status === "holding") {
				if (audioTime >= note.time + (note.duration ?? 0) && heldLanes.has(note.lane)) {
					note.status = "hit";
			...
			if (audioTime <= note.time + JUDGEMENT_WINDOWS.Good / 1e3) break;
			note.status = "miss";
			this.score = applyJudgement(this.score, "Miss");
			events.push({
				judgement: "Miss",
				deltaMs: (audioTime - note.time) * 1e3,
				lane: note.lane
			});
		}
		this.advanceMissCursor();
	...
	advanceMissCursor() {
		while (this.nextMissIndex < this.notes.length && ["hit", "miss"].includes(this.notes[this.nextMissIndex].status)) this.nextMissIndex += 1;
	}
};
//#endregion
//#region src/game/InputManager.ts
...
//#endregion
//#region src/game/NoteManager.ts
var NoteManager = class {
	render(canvas, options) {
		...
		context.restore();
		const visibleStart = options.audioTime - .22;
		const visibleEnd = options.audioTime + travelTime + .25;
		for (const note of options.notes) {
			...
			if (note.type === "hold") {
				...
				context.fillRect(centerX - noteWidth * .32, top, noteWidth * .64, bodyHeight);
			}
			...
			context.fillRect(centerX - noteWidth * .34, y - 2, noteWidth * .68, 3);
			context.restore();
		}
...
var DEFAULT_HEADERS$1 = { "X-Client-Info": `storage-js/2.110.8` };
...
//#endregion
//#region src/services/rankingService.ts
var RECORD_KEY = "afterglow.records.v2";
var SETTINGS_KEY = "afterglow.settings.v1";
var PLAYER_KEY = "afterglow.player-name.v1";
var SCORE_TABLE = "rhythm_scores";
var supabaseUrl = (void 0)?.trim();
var supabaseKey = (void 0)?.trim();
var supabase = supabaseUrl && supabaseKey ? createClient(supabaseUrl, supabaseKey, { auth: {
...
function recordKey(songId, difficulty) {
...
var rankingService = {
	...
	getConfigMessage() {
		return supabase ? "Supabase 글로벌 랭킹 연결됨" : "Supabase 환경변수가 없어 글로벌 랭킹이 비활성화되었습니다.";
	},
	getLocalRecord(songId, difficulty) {
		return safeRead(RECORD_KEY, {})[recordKey(songId, difficulty)];
	},
	saveLocalRecord(songId, difficulty, record) {
		const records = safeRead(RECORD_KEY, {});
	...
	async getLeaderboard(songId, difficulty, limit = 20) {
		if (!supabase) return [];
		const { data, error } = await supabase.from(SCORE_TABLE).select("id,player_name,song_id,difficulty,score,accuracy,max_combo,grade,created_at").eq("song_id", songId).eq("difficulty", difficulty).order("score", { ascending: false }).order("accuracy", { ascending: false }).limit(Math.max(1, Math.min(50, limit)));
...
//#endregion
//#region src/components/CalibrationPanel.tsx
function CalibrationPanel({ currentOffset, onApply }) {
	const [status, setStatus] = (0, import_react.useState)("idle");
	const [beat, setBeat] = (0, import_react.useState)(0);
	const [measured, setMeasured] = (0, import_react.useState)(currentOffset);
	const expectedRef = (0, import_react.useRef)([]);
	const tapsRef = (0, import_react.useRef)([]);
	const contextRef = (0, import_react.useRef)(null);
	(0, import_react.useEffect)(() => {
		const handleKey = (event) => {
			if (status !== "running" || event.repeat || !LANE_CODES.includes(event.code)) return;
			event.preventDefault();
			const now = performance.now();
			const closest = expectedRef.current.reduce((best, expected) => Math.abs(expected - now) < Math.abs(best - now) ? expected : best, expectedRef.current[0] ?? now);
	...
	const start = async () => {
		...
		const audioStart = context.currentTime + .7;
		for (let index = 0; index < 8; index += 1) {
			...
			gain.gain.setValueAtTime(1e-4, audioStart + index * interval / 1e3);
			gain.gain.exponentialRampToValueAtTime(.22, audioStart + index * interval / 1e3 + .006);
			gain.gain.exponentialRampToValueAtTime(1e-4, audioStart + index * interval / 1e3 + .08);
			oscillator.connect(gain).connect(context.destination);
			oscillator.start(audioStart + index * interval / 1e3);
			oscillator.stop(audioStart + index * interval / 1e3 + .09);
		}
		window.setTimeout(() => {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "eyebrow",
						children: "LATENCY TUNER"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "싱크 보정" }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "8번의 테스트 비트에 맞춰 Z, X, C, V 중 편한 키를 누르세요. 평균 입력 지연을 5ms 단위로 계산합니다." }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "current-offset",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "현재 오프셋" }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("strong", { children: [
							currentOffset > 0 ? "+" : "",
							currentOffset,
							"ms"
						] })]
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: `beat-orbit ${status === "running" ? "is-running" : ""}`,
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "beat-core",
						children: status === "running" ? beat : status === "done" ? `${measured}ms` : "READY"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "orbit orbit-one" }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "orbit orbit-two" })
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "calibration-actions",
				children: [
					status !== "running" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						className: "button button-primary",
						onClick: start,
						children: status === "done" ? "다시 측정" : "측정 시작"
					}),
					status === "done" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						className: "button",
						onClick: () => onApply(measured),
						children: "측정값 적용"
					}),
					status === "running" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "pulse-copy",
						children: "비트를 듣고 키를 누르세요"
					})
				]
			})
		]
...
//#endregion
//#region src/components/LeaderboardPanel.tsx
function LeaderboardPanel({ songs, initialSongId, refreshKey = 0 }) {
	const [songId, setSongId] = (0, import_react.useState)(initialSongId ?? songs[0]?.id ?? "");
	const [difficulty, setDifficulty] = (0, import_react.useState)("normal");
	const [entries, setEntries] = (0, import_react.useState)([]);
	const [status, setStatus] = (0, import_react.useState)("랭킹을 불러오는 중입니다.");
	(0, import_react.useEffect)(() => {
		let cancelled = false;
		const load = async () => {
			if (!songId) return;
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "eyebrow",
					children: "GLOBAL SCORE NETWORK"
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					id: "leaderboard-title",
					children: "Leaderboard"
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: `cloud-indicator ${rankingService.isCloudConfigured() ? "online" : ""}`,
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {}), rankingService.getConfigMessage()]
				})]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "leaderboard-filters",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { children: ["TRACK", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("select", {
					value: songId,
					onChange: (event) => setSongId(event.target.value),
					children: songs.map((song) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
						value: song.id,
						children: song.title
					}, song.id))
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { children: ["DIFFICULTY", /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
					value: difficulty,
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "ACC" }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "COMBO" }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "GRADE" })
						]
					}),
					entries.map((entry, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: `leaderboard-row ${index < 3 ? `podium podium-${index + 1}` : ""}`,
						role: "row",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: String(index + 1).padStart(2, "0") }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: entry.playerName }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: entry.score.toLocaleString() }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [entry.accuracy.toFixed(2), "%"] }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: entry.maxCombo }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: entry.rank })
						]
					}, entry.id)),
...
//#endregion
//#region src/components/ScoreSubmission.tsx
function ScoreSubmission({ score, onSubmitted }) {
	const [name, setName] = (0, import_react.useState)(() => rankingService.getPlayerName());
	const [status, setStatus] = (0, import_react.useState)(rankingService.isCloudConfigured() ? "닉네임을 입력해 글로벌 랭킹에 등록하세요." : rankingService.getConfigMessage());
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
				...
				value: name,
				maxLength: 16,
				disabled: done || !rankingService.isCloudConfigured(),
				placeholder: "PLAYER NAME",
				onChange: (event) => setName(event.target.value)
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
				className: "button button-primary",
				disabled: done || submitting || !rankingService.isCloudConfigured(),
				onClick: submit,
				children: done ? "REGISTERED" : submitting ? "UPLOADING" : "REGISTER SCORE"
			})]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: status })]
	});
}
//#endregion
//#region src/components/SongSelect.tsx
function SongSelect({ songs, selectedId, onSelect }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		className: "song-select-shell",
		"aria-labelledby": "song-select-title",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "song-select-heading",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
					className: "eyebrow",
					children: ["MUSIC ARCHIVE / ", songs.length.toString().padStart(2, "0")]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", {
					id: "song-select-title",
					children: "Select track"
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "곡을 선택한 뒤 난이도와 싱크를 조절해 플레이하세요. `public/songs.json`에 항목을 추가하면 이 화면에 자동으로 표시됩니다." })
			]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "song-catalog",
			children: [songs.map((song, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
				className: `song-card ${selectedId === song.id ? "selected" : ""}`,
				style: { "--song-accent": song.accent },
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "song-art",
					"aria-hidden": "true",
					children: [
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "eyebrow",
							children: "AVAILABLE TRACK"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: song.title }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: song.artist }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "song-specs",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: song.bpm }), " BPM"] }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: formatDuration(song.duration) }), " LENGTH"] }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: "3" }), " DIFFICULTIES"] })
							]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							className: "button button-primary",
							onClick: () => onSelect(song),
							children: selectedId === song.id ? "SELECTED · PLAY" : "SELECT & PLAY"
						})
					]
				})]
			}, song.id)), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", {
				className: "song-card coming-soon",
				"aria-disabled": "true",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "eyebrow",
							children: "EXPANSION SLOT"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "New signal" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "곡과 채보를 추가하면 자동으로 활성화됩니다." })
					]
				})]
			})]
		})]
...
//#endregion
//#region src/components/RhythmGame.tsx
function RhythmGame() {
	const canvasRef = (0, import_react.useRef)(null);
	const audioRef = (0, import_react.useRef)(null);
	const engineRef = (0, import_react.useRef)(new GameEngine());
	const inputRef = (0, import_react.useRef)(new InputManager());
	const rendererRef = (0, import_react.useRef)(new NoteManager());
	const countdownTimerRef = (0, import_react.useRef)(null);
	const latestEventRef = (0, import_react.useRef)(null);
	const [mode, setMode] = (0, import_react.useState)("songs");
	const [phase, setPhase] = (0, import_react.useState)("idle");
	const [songs, setSongs] = (0, import_react.useState)([]);
	const [selectedSong, setSelectedSong] = (0, import_react.useState)(null);
	const [difficulty, setDifficulty] = (0, import_react.useState)("normal");
	...
	const [message, setMessage] = (0, import_react.useState)("채보와 음악을 준비하고 있습니다.");
	const [judgement, setJudgement] = (0, import_react.useState)(null);
	const [revision, setRevision] = (0, import_react.useState)(0);
	const [clock, setClock] = (0, import_react.useState)(0);
	const [leaderboardRefresh, setLeaderboardRefresh] = (0, import_react.useState)(0);
	...
	(0, import_react.useEffect)(() => {
		let cancelled = false;
		fetch("./songs.json", { cache: "no-store" }).then((response) => {
	...
	const showEvent = (0, import_react.useCallback)((event) => {
	...
	const finishGame = (0, import_react.useCallback)(() => {
	...
	(0, import_react.useEffect)(() => {
		...
		const handleKeyDown = (event) => {
			...
			event.preventDefault();
			if (phase === "playing") {
				const gameTime = audio.currentTime + settings.audioOffset / 1e3 + (chart?.offset ?? 0);
	...
	(0, import_react.useEffect)(() => {
		...
		const render = (timestamp) => {
			...
			if (canvas) rendererRef.current.render(canvas, {
				audioTime: gameTime,
	...
	const accuracy = engine.getAccuracy();
	const best = (0, import_react.useMemo)(() => typeof window === "undefined" || !selectedSong ? void 0 : rankingService.getLocalRecord(selectedSong.id, difficulty), [
		difficulty,
		revision,
		selectedSong
	]);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("a", {
						...
						href: "#top",
						"aria-label": "Afterglow 홈",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "brand-mark",
							children: "A"
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "AFTERGLOW" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "NEURAL RHYTHM SYSTEM" })] })]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("nav", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								...
								onClick: () => changeMode("songs"),
								children: "SONG SELECT"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								...
								onClick: () => changeMode("calibration"),
								children: "SYNC"
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: `system-status ${loaded ? "online" : ""}`,
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {}),
							" ",
			...
			mode === "songs" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SongSelect, {
				songs,
				selectedId: selectedSong?.id ?? null,
				onSelect: selectSong
			}),
			mode === "play" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "game-layout",
				id: "top",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", {
						className: "hud-column hud-left",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "song-meta",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
										className: "eyebrow",
										children: ["NOW PLAYING / ", Math.max(1, songs.findIndex((song) => song.id === selectedSong?.id) + 1).toString().padStart(2, "0")]
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: selectedSong?.title ?? "Select a track" }),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: selectedSong?.artist ?? "MUSIC ARCHIVE" })
								]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "metric-stack",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(HudMetric, {
										label: "SCORE",
										value: score.score.toLocaleString().padStart(7, "0"),
										accent: true
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(HudMetric, {
										label: "COMBO",
										value: score.combo.toString().padStart(3, "0")
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(HudMetric, {
										label: "MAX COMBO",
										value: Math.max(score.maxCombo, best?.maxCombo ?? 0).toString().padStart(3, "0")
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)(HudMetric, {
										label: "ACCURACY",
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								...
								children: [
									...
									"Good",
									"Miss"
								].map((key) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: key.toUpperCase() }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: score.counts[key].toString().padStart(3, "0") })] }, key))
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
						className: "playfield-panel",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "stage-index",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "STAGE 01" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: difficulty.toUpperCase() })]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("canvas", {
								ref: canvasRef,
								className: "game-canvas",
								"aria-label": "4레인 리듬게임 플레이 화면"
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: `judgement-pop ${judgement ? judgement.judgement.toLowerCase() : ""}`,
								"aria-live": "polite",
								children: judgement && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: judgement.judgement }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
									judgement.deltaMs > 0 ? "+" : "",
									Math.round(judgement.deltaMs),
									"ms"
								] })] })
							}),
							phase === "countdown" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "countdown",
								"aria-live": "assertive",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "GET READY" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: countdown || "GO" })]
							}),
							phase === "idle" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "idle-prompt",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "PRESS START TO CONNECT" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "key-row",
									children: LANE_LABELS.map((key, lane) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("kbd", {
										style: { "--lane-color": LANE_COLORS[lane] },
										children: key
									}, key))
								})]
							}),
							phase === "paused" && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "pause-overlay",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "PLAYBACK SUSPENDED" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "PAUSED" })]
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", {
						className: "hud-column hud-right",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "Game setup" })]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
								className: "control-field",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: "DIFFICULTY" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: difficulty.toUpperCase() })] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
									value: difficulty,
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RangeControl, {
								...
								step: .05,
								suffix: "×",
								onChange: (noteSpeed) => setSettings((value) => ({
									...value,
									noteSpeed
								}))
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RangeControl, {
								...
								onChange: (volume) => setSettings((value) => ({
									...value,
									volume
								}))
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(RangeControl, {
								...
								step: 5,
								suffix: "ms",
								display: `${settings.audioOffset > 0 ? "+" : ""}${settings.audioOffset}ms`,
								onChange: (audioOffset) => setSettings((value) => ({
									...value,
									audioOffset
								}))
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										className: "button button-primary",
										onClick: startGame,
										disabled: !loaded || phase === "countdown",
										children: phase === "results" ? "RETRY RUN" : "START RUN"
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
										className: "button",
										onClick: togglePause,
							...
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								role: "status",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {}), message]
							})
						]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("footer", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatTime(clock) }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "progress-track",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("i", { style: { width: `${Math.min(100, progress)}%` } })
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: formatTime(audio?.duration ?? 0) }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "bpm-chip",
								children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: chart ? Math.round(chart.bpm) : "---" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "BPM" })]
							})
						]
					})
				]
			}),
			mode === "ranking" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LeaderboardPanel, {
				songs,
				initialSongId: selectedSong?.id,
				refreshKey: leaderboardRefresh
			}),
			mode === "editor" && audio && chart && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartEditor, {
				audio,
				chart,
				onChartChange: (next) => {
					setChart(next);
			...
			mode === "calibration" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CalibrationPanel, {
				currentOffset: settings.audioOffset,
				onApply: (audioOffset) => {
					setSettings((value) => ({
						...value,
						audioOffset
					}));
					setMessage(`싱크 오프셋을 ${audioOffset}ms로 저장했습니다.`);
					changeMode("play");
				}
			}),
			phase === "results" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
					...
					children: [
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "rank-orb",
							children: engine.getRank()
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", {
							id: "result-title",
							children: "Signal synchronized"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { children: [
							difficulty.toUpperCase(),
							" · ",
							chart?.notes.length.toLocaleString(),
							" NOTES"
						] }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "result-score",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "FINAL SCORE" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: score.score.toLocaleString() })]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResultMetric, {
									label: "MAX COMBO",
									value: score.maxCombo.toString()
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResultMetric, {
									label: "PERFECT",
									value: score.counts.Perfect.toString()
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResultMetric, {
									label: "GREAT",
									value: score.counts.Great.toString()
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResultMetric, {
									label: "GOOD",
									value: score.counts.Good.toString()
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ResultMetric, {
									label: "MISS",
									value: score.counts.Miss.toString()
								})
							]
						}),
						selectedSong && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ScoreSubmission, {
							score: {
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "DESKTOP LINK REQUIRED" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "정확한 키 입력과 4레인 플레이를 위해 데스크톱 브라우저에서 열어 주세요." })]
			})
		]
	});
}
function HudMetric({ label, value, accent = false }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: `hud-metric ${accent ? "accent" : ""}`,
...
function RangeControl({ label, value, min, max, step, suffix = "", display, onChange }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
		className: "control-field",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("b", { children: label }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: display ?? `${value.toFixed(2)}${suffix}` })] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
			type: "range",
			min,
			max,
			step,
