...
| 일시정지 | 우측 `PAUSE` |
| 타격 효과음 | `Z`, `X`, `C`, `V` 모두 동일한 짧은 스네어 드럼 사운드 재생 |
| 타격 이펙트 | 노트를 맞히면 해당 레인의 판정 위치에서 충격파·파티클 표시 |
...
닉네임은 `rhythm_players`에 대소문자를 구분하지 않는 고유 이름으로 등록됩니다. `RANKING` 화면은 같은 플레이어의 곡·난이도별 최고 점수 하나만 표시하며, `내 점수 보기` 버튼으로 현재 플레이어의 최고 기록과 실제 순위를 확인할 수 있습니다. 기존에 스키마를 설치했다면 업데이트된 `supabase/schema.sql`을 SQL Editor에서 다시 한 번 전체 실행하세요. 기존 점수는 삭제되지 않습니다.

공개 클라이언트 방식은 RLS와 값 범위로 오용을 제한하지만 완전한 본인 확인이나 치팅 방지는 아닙니다. 강한 계정 소유권이 필요하면 Supabase Auth와 Edge Function을 함께 사용하세요.
...
볼륨, 노트 속도, 오디오 오프셋, 곡·난이도별 개인 최고 기록과 선택한 닉네임은 `localStorage`에 저장됩니다. 따라서 같은 브라우저에서 다시 접속하면 기존 닉네임으로 자동 입장합니다. 브라우저 데이터 삭제, 시크릿 모드, 다른 기기에서는 이 정보가 이어지지 않습니다. 글로벌 기록과 닉네임 예약은 `src/services/rankingService.ts`를 통해 Supabase에 저장됩니다.
